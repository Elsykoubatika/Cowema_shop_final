...
const LoyaltyCardSection: React.FC = () => {
  const { isAuthenticated, user } = useUnifiedAuth();
  const navigate = useNavigate();
  
  const currentUserPoints = user?.loyaltyPoints || 0;
  const { loyaltyLevels, currentLevel, nextLevelInfo, loading } = useDynamicLoyalty(currentUserPoints);
  const [activeLevel, setActiveLevel] = useState<number>(0);
  
  React.useEffect(() => {
    if (isAuthenticated && user && currentLevel) {
  ...
  }, [isAuthenticated, user, currentLevel, loyaltyLevels]);
  ...
  // Get an urgency message to motivate the user
  const getUrgencyMessage = () => {
    if (!user || !currentLevel || !nextLevelInfo) return "";
    
    if (currentLevel.name === "Diamant") {
    ...
    return `Plus que ${daysLeft} jours pour atteindre le niveau ${nextLevelInfo.nextLevel} et obtenir une remise supplémentaire !`;
  };
      ...
      <div className="container-cowema max-w-4xl">
        <div className="section-title mb-4 text-center">
          ...
          <p className="text-cowema-lightText max-w-xl mx-auto text-sm">
            Gagnez des points à chaque achat et profitez d'avantages exclusifs ! 1 000 FCFA = 1 point. 
        ...
        <div className="grid grid-cols-12 gap-4 items-center">
          <div className="col-span-12 md:col-span-7">
            ...
            <div className="bg-white p-3 rounded-lg shadow-sm">
              ...
              {isAuthenticated && user && currentLevel && nextLevelInfo ? (
                <div className="mt-3">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs text-gray-600">Niveau actuel: {currentLevel.name}</span>
                    <span className="text-xs font-medium">{currentUserPoints} points</span>
                  </div>
                  ...
                  <div className="text-xs text-gray-600 mb-2">
                    {nextLevelInfo.pointsNeeded > 0 ? (
                      <p>Plus que <span className="font-medium">{nextLevelInfo.pointsNeeded}</span> points pour le niveau {nextLevelInfo.nextLevel}!</p>
                    ) : (
            ...
            <div className={`aspect-[1.6/1] rounded-xl overflow-hidden shadow-lg rotate-3 transform transition-transform hover:rotate-0 relative z-10 ${loyaltyLevels[activeLevel]?.color.replace('bg-', 'bg-').replace('-100', '-600')}`}>
              <div className="absolute inset-1 bg-white rounded-lg p-4 flex flex-col justify-between">
                ...
                <div className="text-xs">
                  <div className="opacity-70">1 000 FCFA = 1 point</div>
                </div>
              </div>
            </div>
          </div>
